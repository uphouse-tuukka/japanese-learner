...
### Completed TranslationExercise view-model helper batch

**Completed on:** 2026-05-08

**Commit:** `chore: extract translation exercise view model helpers`

**Validation:** TDD RED was directly observed before implementation: `npm test -- src/lib/components/exercises/translation-exercise-view-model.test.ts` failed because `./translation-exercise-view-model` did not exist. GREEN targeted validation passed with `5` tests. `npm run check` passed with `0` errors and `0` warnings during implementation/review. `git diff --check` passed. Added-line secret/security scan passed. `npm run validate:ci` passed after the implementation and documentation updates, including format check, Svelte check, ESLint, Vitest (`303` tests / `40` files), and production build. Existing Vercel optional dependency warnings were unchanged.

**Review status:** independent spec-compliance review passed with no gaps. Independent code-quality/security review approved with no critical, important, or minor issues. No browser visual check was performed because the slice preserved the existing DOM structure, CSS/classes/styles, InlineAudio placement, and interaction flow while moving pure derived display values into a helper module.
