...
1. Continue API/profile boundary hardening with Task 8.3: migrate the next high-risk write API one route per task/commit with route tests first. Do not redo the selected-user helper, `writing-toggle`, `level`, or `practice/generate` migrations. Good next candidates are `src/routes/api/session/generate/+server.ts` or the mission write routes.
