...
- [ ] Task 3 — Wire Coverage Evidence and Learning Journal into session generation prompt/input with selected category rails. Handoff: start by querying completed AI sessions plus joined exercise results in `/api/session/generate`, fetch the user's current `progressJournal`, build Coverage Evidence with `buildCoverageEvidence()`, and pass `coverageEvidence.promptSnapshot` plus the bounded Learning Journal into `generateSessionPlan()`.
