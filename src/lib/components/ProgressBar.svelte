<script lang="ts">
let { current, total, label = 'Progress' } = $props<{ current: number; total: number; label?: string }>();
...
<div class="progress-track" role="progressbar" aria-valuemin={0} aria-valuemax={safeTotal} aria-valuenow={clampedCurrent}>
