...
const ALLOWED_LEVELS: UserLevel[] = ['absolute_beginner', 'beginner', 'pre_intermediate'];
...
export async function getUsers(): Promise<User[]> {
  ...
  console.warn('[users] fetched users', { count: users.length });
  return users;
}
...
export async function getUser(userId: string): Promise<User | null> {
  ...
  console.warn('[users] fetched user', { userId: id, found: Boolean(user) });
  return user;
}
...
export async function createUser(input: { name: string; level?: UserLevel }): Promise<User> {
  ...
  console.warn('[users] created user', {
    id: created.id,
    level: created.level,
...
export async function updateUserLevel(userId: string, level: UserLevel): Promise<User> {
  ...
  console.warn('[users] updated level', { userId: id, level: validatedLevel });
  return updated;
}
