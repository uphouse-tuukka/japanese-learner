...
<td data-label="Accuracy">
    <span class="accuracy-value" class:high={item.accuracy >= 80} class:low={item.accuracy > 0 && item.accuracy < 50}>
        {item.accuracy > 0 ? `${item.accuracy}%` : '-'}
    </span>
</td>
<td data-label="Exercises" class="cell-exercises">{item.exerciseCount} practice items</td>
</tr>
{/each}
</tbody>
</table>
